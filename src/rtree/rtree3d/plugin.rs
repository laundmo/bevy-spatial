...
use super::rtree_obj::TreeAccess3D;

pub struct RTreePlugin3D<TComp, Params> {
...
impl<TComp, Params> Default for RTreePlugin3D<TComp, Params> {
    fn default() -> Self {
        RTreePlugin3D {
...
impl<TComp, Params> Plugin for RTreePlugin3D<TComp, Params>
where
    TComp: Component + Send + Sync + 'static,
    Params: RTreeParams + 'static,
{
    fn build(&self, app: &mut App) {
        let tree_access = TreeAccess3D::<TComp, Params> {
        ...
        app.insert_resource(tree_access)
            .add_startup_system_to_stage(
                ...
                add_added::<TreeAccess3D<TComp, Params>>,
            );
        ...
        if let Some(step) = self.timestep {
            ...
            app.add_system_set_to_stage(
                ...
                SystemSet::new()
                    ...
                    .with_system(add_added::<TreeAccess3D<TComp, Params>>)
                    .with_system(update_moved::<TreeAccess3D<TComp, Params>>)
                    .with_system(delete::<TreeAccess3D<TComp, Params>>),
            );
        } else {
            ...
            .add_system_to_stage(
                CoreStage::PostUpdate,
                update_moved::<TreeAccess3D<TComp, Params>>,
            )
            .add_system_to_stage(CoreStage::PostUpdate, delete::<TreeAccess3D<TComp, Params>>);
        }
    }
}
